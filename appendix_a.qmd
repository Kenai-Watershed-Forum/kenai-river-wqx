...
-   Results:
    ...
    -   Magnesium: nearly all available observations show dissolved metals \< total metals, with the exception of two sites near the Kenai River mouth (RM 0 NNC and RM 1.5). At both sites the difference is small and may be within measurement uncertainty.
...
As of the time of this writing, it is unclear why dissolved metals values exceed total metals values for calcium at all measured sites, and for magnesium at two sites near the river mouth (RM 0 NNC and RM 1.5). Iron behaves as expected (total \> dissolved) at all measured sites. This discrepancy for calcium and magnesium warrants follow-up in future sampling years.
...
Work in progress — results to be interpreted following review of output files above.
...
# --> TO DO::: dveelop import config to accommodate this csv
...
# The download contain extensive column info about well characteristics. No wells are included as part of this study, but we kept them in the import configuration for consistency's sake. TBD if this is necessary.
...
# working here 12/6/23 - modify/adapt/delete code below to accomplish above 2 goals
# rename as "export_dat" once script successful & complete
...
export_dat %<>%  
   mutate(
    ...
    ### answer in progress here 12/15/23
    # question - can we have mre than one result detection condition column
    # in the CDX import configuration, it appears that multiple columns can have the identical name. This will not work in R. Try: in export_dat, create detection limit columns designated as "1" and "2", then remove these for the import/upload
...
# need to modify this to include MonitoringLocationIdentifier
wqx_colnames <- read_excel("other/input/wqx_templates/AWQMS_KWF_Baseline_2021.xlsx", sheet = "KWF Baseline AWQMS Template") %>%
  colnames()
