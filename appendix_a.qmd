...
-   *Fecal Coliform*
    ...
    -   Given that 3 out of 4 RPD values for fecal coliform values are \>60% (project goal is 20%), field data values for the 2021 dataset may vary from actual environmental conditions. As a result KWF will submit 2021 fecal coliform data to the EPA WQX as "flagged". Fecal coliform data will be evaluated in this report and assessed against state standards, but given the high RPD values, 2021 data will likely not be considered in the context of potential regulatory exceedences.
...
-   *Selected Metals (As, Cd, Cr, Cu, Pb, Zn)*
    ...
    -    Note: beginning in Summer 2022, KWF initiated a series of changes to the QAPP that will better facilitate QA/QC decisions regarding Total Metals and Dissolved metals results.
...
As of `r Sys.Date()` "Total Nitrate/Nitrite-N" values will be flagged for the spring 2021 dataset. (Spring only and not summer because no matrix spike recovery exedences were observed from the summer 2021 lab results). The matrix spike recovery fails indicate that reported lab values may be 2% - 7% above actual field values. At a later date we will evaluate these results in the context of year 2000 - present data to help evaluate the relevancy of this flag.
...
```{r echo = F}
total_samples_collected_2021_summary_param <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
...
# 8/23/2023 working here
# next step: use list in other/input/AQWMS/analytes_list_manual_edit.csv to reduce list and make completeness calcs
