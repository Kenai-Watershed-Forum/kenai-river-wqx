...
Water quality data from this project is ultimately destined for the Environmental Protection Agency's Water Quality Exchange (EPA WQX), formerly EPA STORET. The process of transferring these data to the higher-level EPA repository is referred to as data "uplift."
...
In this appendix we will collate 2021 laboratory data from several sources into a single spreadsheet document with a consistent format. The desired end format is a spreadsheet template provided by the EPA Water Quality Exchange. These template files are available to download at <https://www.epa.gov/waterdata/water-quality-exchange-web-template-files>.

Once the data is collated, it will be evaluated according to a Quality Assurance Checklist provided by the Alaska Department of Environmental Conservation Soldotna office. Field observations that do not meet the quality assurance standards described in the evaluation checklist will not be uplifted to the EPA WQx.

Data that has been uplifted to the EPA WQX is evaluated biannually by the Alaska Department of Environmental Conservation (ADEC) in their [Integrated Water Quality Monitoring and Assessment Report](https://dec.alaska.gov/water/water-quality/integrated-report/)[^appendix_a-1].
...
modify code to retain all lab dat for later step? (incl surr, spikes, etc)
...
# These lab-only data will be evaluated at a later step. See excel file "other/input/AQWMS/sample_type_abbreviations_manual_edit.xlsx" for sample_type naming schema.
...
## BUG ID here - no content in "sample" column for FC samples after join
## solution: rename sample column after lab name mutate
```
...
######## The SWWTP TSS SOP at "other\documents\references". The SOP does not describe the the kind of QA we need for baseline, as described in our QAPP
...
####### Bottom line: need to report this gap in planned QA practices, and re-instate for 2023 !!!
...
# temporarily reduce DF width to visually examine  errors described below
...
# note 5/18/2022 -- some samples from 5/11/2021 no name creek have "PS" for sample type but have "Field Duplicate" for sample_condition. need to fix. 

# also need to disambiguate "PS" from "SMPL" (the latter is the ALS lab designation)
# exploring around line 474 to diagnose
# 
## "sample" column
# seeing some inconsistency in sample_name formats too (with _)?
...
# some NNC project sample names have a trailing _
# --> may not be necesary to include in final format .... do we even need to correct this column?

# steps to rectify:
## 1.) re-examine ALL of the manual-edit csv matching tables to diagnose site-name inconsistencies; see if the issue arises from the original joins or at some later step
### Check FC
### Check TSS
### 2.) rename SMPL Ca/Mg results as PS in line 1045. (Do so here rather than later on so as not to interfere with other previous global edits/mutates that may refer to "SMPL")
...
The above data sources have been collated in to a single .csv file (available for download) per following guidance in the current Quality Assurance Project Plan.
...
# check emails from sarah to see if this still the most current one
```

```{r}
# stop here as of 2/13/23; will not knit beyond!
knitr::knit_exit()
```
...
<summary>*Show/Hide Code used to Evaluate Overall Project Success*</summary>
...
4.  [**Is the dataset complete and did you receive the expected number of results?**]{.underline} *TBD*. Notes: we acquired the expected number of results in 2021 sampling events. All sites described in the QAPP were visited twice in 2021, and all water sample collections were successfully conducted. However most intrinsic water quality parameters mentioned in the current QAPP were not collected in 2021.
