...
From the above table, we can see that a limited subset of paired values from field duplicates are eligible to be used for RPD calculations. Pairs of measurements must meet LOQ criteria described above ("a" and "b"). For 2021 data, of 69 paired field duplicate observations, 24 of them may be used for RPD calculations.
...
-   It ***was possible*** to calculate RPD values because:
    ...
    -   Project data will be submitted to EPA WQX if RPD values meet project goals outlined in the QAPP
-   It ***was not possible*** to calculate RPD values because:
    ...
    -   Project data will be submitted to EPA WQX if both paired values are non-detect
    
KWF sent a memo to the Alaska Department of Environmental Conservation on August 22, 2023 with an inquiry about best practices for interpreting and applying RPD values as described above. The memo includes a visual flow chart and is available for download below.
...
*Special note on 2021 fecal coliform data*

Given that 3 out of 4 RPD values for fecal coliform values are >60% (project goal is 20%), field data values for this dataset generally may be suspect.  As a result KWF does not plan to submit 2021 fecal coliform data to the EPA WQX for 2021.

Fecal coliform data will be evaluated in this report and assessed against state standards, but given the high RPD values, 2021 data will not be considered in the context of potential regulatory exceedences.
...
Based in this information, we will decide which results to flag and not submit to EPA WQX. Further discussion will be present here in final report, along with a visual flow chart of the logic for when data is excluded and included.
...
# source results file, re-save as "flagged file" in same folder; for each step that involves flagging
# in final output, flagged results will be excluded prior to EPA uplift

# read in data formatted for export
export_dat <- read.csv('other/output/aqwms_formatted_results/2021_kwf_baseline_results_aqwms.csv') %>%
  clean_names()

# read in rpd check data
export_dat_flagged <- export_dat %>%
  mutate(resultFlag = case_when(
    characteristic_name == "Fecal Coliform" ~ "Y"
  ))

write.csv(export_dat_flagged,"other/output/aqwms_formatted_results/2021_export_data_flagged.csv")

# WORKING HERE: apply flags based on RPD values in earlier csv, then join here. don't need to make it available for re-download until end of gauntlet with remaining qapp quetsions.
...
As of `r Sys.Date()` we do not intend to reject "Total Nitrate/Nitrite-N" values from the 2021 dataset. The matrix spike recovery fails indicate that reported lab values may be 2% - 7% above actual field values. At a later date we will evaluate these results in the context of year 2000 - present data to help evaluate the validity of this decision, and remove the results from the EPA WQX if appropriate. We would also like to consult with ADEC to determine if these laboratory-identified discrepancies are grounds for rejecting field observations.
...
As of `r Sys.Date()` we would like to consult with ADEC to determine if these discrepancies discussed in questions #18, #19, and #20 are grounds for rejecting field observations. If so, results will be flagged as appropriate and not submitted to EPA WQX.
...
In 2021, KWF initiated as standard practice the receipt of machine-readable EDD files for contracted laboratories, in addition to PDF results. The EDD files are being reviewed as part of this QA/QC process, which was initiated in Winter 2022.

<br>

**23.) Is the dataset complete and did you receive the expected number of results?**

From the discussion and results in question #3, deviations are evident between planned and actual results reported. The reasons for the deviations are known and are attributable to the two factors discussed in question #3.

<br>

**24.) Was the data collected representative of environmental conditions?.**

The data collected in this project is representative of seasonal hydrological conditions on the fieldwork days. Conditions vary instantaneously thus this dataset serves to provide representative longitudinal snapshots of the watershed across space on two dates.
...
From the QAPP, Completeness Measure A: "the primary number of samples collected divided by the useable number of samples submitted to ADEC with a goal of 85% completeness."
...
# procedure: based on discussion in above questions, decide what is flagged as "unusable." THEN return to this quetsion to calculate completeness measure A. adapt from existing code in "appendix_a_temp.qmd"
...
**XX.) Does project meet Completeness Measure B criteria?**
