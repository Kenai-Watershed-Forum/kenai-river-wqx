...
# summer 2021 has 
# somewhere above here, spring 2021 Ca and Mg data is removed. but it re-appears later apparently?!
z <- dat %>%
  filter(analyte %in% c("Calcium, Total","Magnesium, Total", "Calcium","Magnesium")) %>%
...
```{r, echo = F}
xfun::embed_file('other/output/aqwms_formatted_results/2021_kwf_baseline_results_aqwms.csv', text = "Download All Provisional 2021 Kenai River Baseline Water Quality Monitoring Results, Formatted for EPA WQX import")
...
```{r, echo = F}
xfun::embed_file('other/documents/AQWMS_documents/Kenai_Baseline_Data_Evaluation_Checklist_20230331.xlsx', text = "Download 2021 Draft Kenai Baseline Data Evaluation Checklist Template")
...
# MOVE THIS STEP TO LAST AFTER ALL QUESTIONS THAT COULD PRODUCE A FLAG

# doing now: 12/4/23 11:20 - check to see if any other questions generate flags
...
flags_2021 <- read.csv('other/input/AQWMS/2021_data_flag_decisions.csv') %>%
  select(-notes) %>%
  transform(activity_start_date = mdy(activity_start_date))
...
write.csv(export_dat,"other/output/aqwms_formatted_results/2021_export_data_flagged.csv", row.names = F)
...
# this step is already accomplished in a previous code block, which imports and joins "other/input/AQWMS/2021_data_flag_decisions.csv" to the parameters chosen in this discussion.
...
analytes_2021 <- read.csv("other/input/AQWMS/analytes_list_manual_edit.csv")
colnames(analytes_2021) <- c("characteristic_name","a","param_in_qapp") 
analytes_2021 %<>% select(-a)

# BY PARAMETER
# summarise flags by parameter
completeness_summary_param <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
...
completeness_summary_site <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
completeness_summary_project <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
cmb_by_method <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
cmb_by_site <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
export_dat <- read.csv("other/output/aqwms_formatted_results/2021_export_data_flagged.csv")
...
# re-write export_dat
write.csv(export_dat,"other/output/aqwms_formatted_results/2021_export_data_flagged.csv")
...
```{r echo = F}
# remove all flagged observations from 2021 dataset
...
## use matched data at "other/input/AQWMS/AQWMS_matching_table.xlsx
site_name_match <- read_excel("other/input/AQWMS/AQWMS_template_matching_table.xlsx", sheet = "adec_site_names") %>%
  remove_empty()
...
# export final formatted AQWMS results to external csv
write.csv(export_dat,"other/output/aqwms_formatted_results/results_activities.csv",row.names = F)
...
#write.csv(project,"other/output/aqwms_formatted_results/project.csv", row.names = F)
...
# write.csv(station,"other/output/aqwms_formatted_results/station.csv", row.names = F)
```
...
```{r echo = F, eval = F}
# make corrections to existing EPA CDX data

# in winter 2025 we discovered a series of updates and corrections needed for both 2000 - 2013 data as well as recently uploaded 2021 data. these corrections are detailed and executed here. notesa re listed in a google doc at https://docs.google.com/document/d/1p1fsh2dVfvIWNmRrpjteOTyLPSEeMYoQDRk5XhKTcNk/edit?tab=t.0
...
# Issue 1 - Units for specific conductance for spring 2013 are all in mS/cm rather than uS/cm. However the range of values for spring 2013 are not totally out range of existing values, so its possible it’s just an error in recording the unit name rather than the actual units. Look in context of existing values to determine

# let's examine if these spring 2021 values are within range of other existing values

# we examined the values in a simple ggplotly, as well as in the original data sheets from 2013. Both indicate the units in WQX shuold be uS/cm rather than mS/cm for spring 2013 specific conductance. we will apply the correction here.
...
# Issue 2 - Some metals values for summer 2021 appear to be unrealistic (Mg,Ca,Fe)
# lets examine the 2021 values in context of all other values
p <- narrowresult %>%
  filter(CharacteristicName %in% c("Iron","Magnesium","Calcium")) %>%
  ggplot(aes(ymd(ActivityStartDate),as.numeric(ResultMeasureValue))) +
  geom_point() +
  facet_grid(CharacteristicName~.)
p

# examine units and sample fractions
z <- p %>%
  select(CharacteristicName,ResultMeasure.MeasureUnitCode,ResultSampleFractionText) %>%
  distinct()

# from examining the data 2000 - 2021, we see that for Ca, Mg, and Fe data exist as either a.) dissolved, which is denoted as "Dissolved" or "Filtered, field", or b.) total, which is denoted as either "Total Recoverable" or "Unfiltered."

# prior to 2021, we did not have data on dissolved Fe, Ca, Mg

# segregating by fraction, lets see if values are still within reasonable range. if not, maybe unit corrections/conversions are needed

p1 <- narrowresult %>%
  filter(CharacteristicName %in% c("Iron","Magnesium","Calcium")) %>%
  ggplot(aes(ymd(ActivityStartDate),as.numeric(ResultMeasureValue))) +
  geom_point() +
  facet_grid(CharacteristicName~ResultSampleFractionText)

ggplotly(p1)

# from examining plot p1, the distinction in units and sample fractions is apparent.
# task: adjusted boxplot function to display only "Total Recoverable" or "Unfiltered" results for Ca and Mg

# Issue 3 - MonitoringLocationName string formats from 2021 do not match those from 2000 - 2013
z <- narrowresult %>%
  distinct(MonitoringLocationIdentifier)

# not a big deal

# Issue 4 - check in to Hardness value for 5/11/2021 at KBL_t_74.0. unrealistic?
z <- narrowresult %>%
  filter(CharacteristicName %in% c("Calcium","Magnesium")) %>%
  filter(ResultSampleFractionText %in% c("Total Recoverable","Unfiltered")) %>%
  filter(ResultMeasureValue != "") %>%
 # filter(ActivityStartDate == "2021-05-11") %>%
  filter(MonitoringLocationIdentifier == "KENAI_WQX-10000032") %>%
  arrange(CharacteristicName,ResultMeasureValue)
  
  group_by(ActivityStartDate, MonitoringLocationIdentifier, CharacteristicName, ResultMeasure.MeasureUnitCode) %>%
  
  # get mean of all duplicate sample
  summarise(avg_value = mean(as.numeric(ResultMeasureValue), na.rm = T))
...
## 1.) see that column name structure in EPA WQX matches that of adec aqwms example
