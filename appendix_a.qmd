...
```{r, 2021 WQX formatting for SGS, echo = F, message = F}
...
# NOTE: IF THE NUMBER/TYPE OF PLANNED SAMPLES IS IDENTICAL TO THE SAMPLES ACTUALLY COLLECTED, THAN COMPLETENESS MEASURES A = COMPLETENESS MEASURES B (?!)

# BY PARAMETER (actually by method)
# unflagged (usable) vs expected results
cmb_by_method <- read.csv('other/output/wqx_formatted/2021_export_data_flagged.csv') %>%
  ...
  # calculate Completeness Measure B for each parameter
...
# prepare table of planned sample summary that includes site names


# repeat methods as above for parameter
cmb_by_site <- read.csv('other/output/wqx_formatted/2021_export_data_flagged.csv') %>%
  ...
  rename(flag_Y = Y,
         flag_N = N) %>%
  left_join(planned_samples_2021_summary_by_site) %>%
  # calculate Completeness Measure B for each SITE
  mutate(cmb = flag_N/expected_ct)%>%
  mutate(cmb = replace_na(cmb,0)) %>%
  adorn_pct_formatting(,,,cmb)

# calculated results here are non-sensical. re-assess what we need in the tables being joined; by site
...
Work still in progress on calculating Completness Measures B as of `r Sys.Date()`
...
# notes on flagging data
# based on above description, use case_when logic to apply flag Y/N as appropriate. 
# code should remain agnostic of parameter and year
# source the results file, re-save as "flagged file" in same folder; do so for each additional future step that involves flagging
...
export_dat <- full_join(export_dat,flags_2021) %>%
  # fill in "flag" column with "N" if not already "Y"
...
# do we need to distinguish between RPD flag and MS flag?


# re-export results w/ rpd flag column
write.csv(export_dat,"other/output/wqx_formatted/2021_export_data_flagged.csv", row.names = F)

# remove existing "export_dat" dataframe
#rm(export_dat)


# Coordinates for JC site are still correct at this step.
# is RM 43 missing longitude?

# 3/28/25 - looking now - are there steps past here that would interfere with the manual flaggiing process in this chunk?
```
...
# assemble all components of upload to one folder location

# renew this code for future years; this is manually edited for now.

# project (hashtagged out for now becuase we modified the csv to prep for upload; changed QAPP status to Y and added website)
...
```{r echo = F, eval = F}
# Corrections to existing EPA CDX data #1 - February 2025

# in this chunk we will conduct the following activities:
# 1) address minor errors that were discovered from downloading all existing EPA WQX data from 2021

# notes are listed in a google doc at https://docs.google.com/document/d/1p1fsh2dVfvIWNmRrpjteOTyLPSEeMYoQDRk5XhKTcNk/edit?tab=t.0

# read in the document downloaded from EPA WQX, which includes some manual edits as described in the ReadMe in the same directory
submitted_dat <- read.csv("other/output/wqx_formatted/final_preparation/results_activities.csv", check.names = F) 


# minor issues with existing data  (NOTE: this is for 2013; not a huge priority)
# Issue 1.1 - Units for specific conductance for spring 2013 are all in mS/cm rather than uS/cm. We examined original records and investigated range of values, as well as in the original data sheets from 2013. Both indicate the units in WQX shuold be uS/cm rather than mS/cm for spring 2013 specific conductance. we will apply the correction here.
submitted_dat <- submitted_dat %>%
  mutate(`Result.Unit` = case_when(
    `Characteristic Name` == "Specific conductance" ~ "uS/cm",
    TRUE ~ `Result Unit`
  ))



# Issue 1.2 - MonitoringLocationName string formats from 2021 do not match those from 2000 - 2013
#z <- dat %>%
#  distinct(Monitoring.Location.Identifier)
# not a big deal, maybe address later

# Issue 1.3 - check in to Hardness value for 5/11/2021 at KBL_t_74.0. unrealistic?
# identified error in manually edited sgs site name matching sheet and corrected

# issues with CDX upload format(s)
# Issue 2.1 - dates need leading zeroes
# Convert the date column (assuming it's named 'date') and format it with leading zeros
# do other date columns need this?
submitted_dat <- submitted_dat %>%
  mutate(`Analysis Start Date` = format(mdy(`Analysis Start Date`), "%m/%d/%Y"))


# Issue 2.2 - "Total" should be removed from Calcium and Magnesium characteristic name. Sample fraction column differentiates filtered and unfiltered
# delete this?
#dat <- dat %>%
#  mutate(`Characteristic Name` = case_when(
#    `Characteristic Name` == "Calcium, Total" ~ "Calcium",
#    `Characteristic Name` == "Magnesium, Total" ~ "Magnesium",
#    TRUE ~ `Characteristic Name`
#  )) 

# export back into final preparation folder
write.csv(submitted_dat,"other/output/wqx_formatted/final_preparation/results_activities.csv", row.names = F)
...
# 1/4/24
# next step: re-start (delete old) EPA CDX import config for results_activities. Somehow I duplicated a bunch of column names. Consider removing all irrelevant columns with no info (all NA) from results_activities, make import config to reflect this


# next steps

# for next years::: take the dataframe at the end of this long script, make so column names/structure conforms to the ADEC example. In the 2021 QA/QC process, it went through the janitor::clean_names process, thus names needed to be fixed again at the end.

## 1.) see that column name structure in EPA WQX matches that of adec WQX example
## 2.) ensure that df export from here matches EPA WQX column names
...
# access all existing KWF water quality data downloaded from EPA CDX on 2/3/2025
# narrowresult <- read.csv("other/input/WQX_downloads/wqp_download_20250225/narrowresult.csv")
# station <- read.csv("other/input/WQX_downloads/wqp_download_20250225/station.csv")
