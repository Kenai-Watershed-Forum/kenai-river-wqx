...
# MOVE THIS STEP TO LAST AFTER ALL QUATESTION THAT COULD PRODUCE A FLAG

# notes on flagging data
# based on above description, use case_when logic to apply flag Y/N as appropriate. 

# remain agnostic of parameter and year
# source results file, re-save as "flagged file" in same folder; for each additional future step that involves flagging
...
# NOTE: IF THE NUMBER/TYPE OF PLANNED SAMPLES IS IDENTICAL TO THE SAMPLES ACTUALLY COLLECTED, THAT COMPLETENESS MEASURES A = COMPLETENESS MEASURES B
...
Kenai Watershed Forum's Organization ID in the EPA Water Quality Exchange is "EPA_WQX." This value is manually confirmed during the upload process.
...
The range of results will be evaluated visually and numerically in each characteristic-specific section of the comprehensive report, relative to past results. Likely outliers will be identified and recorded in spreadsheet (download below). Each likely outlier will have accompanying notes discussing the choice, and a decisions as to whether further statistical investigation is warranted.
