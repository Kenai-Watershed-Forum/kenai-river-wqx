...
From the above table we can see that a limited subset of paired values from field duplicates are eligible to be used for Relative Percent Difference calculations. 

RPD values range from `r min(rpd_check_dat$rpd_pct)` to `r max(rpd_check_dat$rpd_pct`.

Of a total of X paired observations, x pairs pass LOQ criteria and x pairs pass LOQ 2x criteria
