...
# correction applies to summer 2021 observations only.
#
...
# proceed left to right of existing ALS dataframe to make its naming structure match the sgs21 dataframe. Add, remove, modify column names as needed
  ...
  rename(
         ...
         # sample_type ::: not sure where to match with sgs data yet or where to put in WQX output, but is important for qa/qc
...
# Upon visual inspection of site names, we can see that the location names in the WQX template differ slightly from the place names in the SGS report (spelling and name inconsistencies).
...
# See WQX Activity Type column assign distinctions
...
# In preparation for a join to WQX table, we will manually generate a match table csv file that we can use 
...
# generate list of unique site names from 2021 WQX template. These are the names we want in the final product
wqx_sitenames <- read_excel("other/input/wqx_templates/AWQMS_KWF_Baseline_2021.xlsx", sheet = "Monitoring Locations") %>%
...
sitenames21_match <- read_excel("other/input/wqx_templates/sgs_site_names_matching_table_manual_edit.xlsx") %>%
...
rm(sgs21_sitenames,wqx_sitenames,sitenames21_match)
...
wqx_analytical_methods <- read_excel("other/input/wqx_templates/AWQMS_KWF_Baseline_2021.xlsx", sheet = "Analytical Methods") %>%
...
epa_analysis_codes <- inner_join(wqx_analytical_methods,analysis_code_matching_table, by = "epa_analysis_id") %>%
...
# These lab-only data will be evaluated at a later step of QA/QC evaluation. See excel file "other/input/wqx_templates/sample_type_abbreviations_manual_edit.xlsx" for sample_type naming schema.
...
# for this dataframe, retain only non-field sample results for WQX export
dat %<>%
  filter(sample_type %in% c("PS","SMPL","TB")) %>%
...
swwtp_spring21_site_matching <- read_excel("other/input/wqx_templates/swwtp_site_names_matching_table_manual_edit.xlsx") 

# join
# modify join table to contain "sample" column w/ format RM_0_No_Name_Creek
swwtp_spring21 %<>%  
...
write.xlsx(taur_summer21_sites, "other/input/wqx_templates/taurianen_site_names_matching_table.xlsx")
...
taur_summer21_sites <- read_excel("other/input/wqx_templates/taurianen_site_names_matching_table_manual_edit.xlsx") 
...
```{r, echo = F}
embed_if_html('other/input/2021_wqx_data/spring_2021_wqx_data/SWWTP/KRWF TSS MONITORING 05-11-21.xlsx', text = "Download Original Spring 2021 Total Suspended Solids Results from SWWTP.xlsx")
```

```{r, echo = F}
embed_if_html('other/input/2021_wqx_data/summer_2021_wqx_data/SWWTP/KRWF TSS MONITORING 07-28-21.xlsx', text = "Download Original Summer 2021 Total Suspended Solids Results from SWWTP.xlsx")
```
...
write.csv(swwtp_tss_sitenames,"other/input/wqx_templates/swwtp_tss_sitenames.csv",row.names = F)
...
####### Bottom line: need to re-instate recommended QA for TSS for 2023. This topic has been added to the agenda of the technical advisory committee meeting March 20th, 2023.
...
## read in coordinates
site_coords <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "site_coordinates") %>%
  remove_empty()
...
result_sample_fraction <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "result_sample_fraction") %>%
...
# The fraction join above keys on analytical_method, but TSS and FC use long
# method strings that don't match the lookup table keys. Assign their fractions
# explicitly. All other parameters are handled by the join above.
# - TSS (SM 2540-D): "Suspended" is the correct WQX domain value for a
#   settled/filtered solids measurement on an unfiltered whole-water sample.
# - Fecal Coliform (9222D): "None" is the correct WQX domain value when no
#   sample fraction applies (whole-water biological parameter).
dat <- dat %>%
...
result_detection_condition <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "result_detection_condition") %>%
  clean_names() %>%
...
## read in table
chemical_preservative <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "chemical_preservative") %>%
...
bottle_type_color <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "sample_container_type_color") %>%
  select(-description) %>%
...
write.csv(data.frame(unique(dat$analyte)),"other/input/wqx_templates/analytes_list.csv", row.names = F)
...
# temporarily reduce DF width to visually examine  errors described below; see "temp" file in wqx_templates folder
...
#write.csv(z, "other/input/wqx_templates/temp.csv")
...
## Note: at this stage there are lots of format issues with conventions in the "sample" column, but this column is not retained in the final data export so it is not an urgent issue. For reference, these issues include:
...
z1 <- dat %>%
  filter(analyte %in% c("Magnesium","Calcium","Calcium, Total","Magnesium, Total"),
         collect_date == "2021-5-11")


############ g.) prepare final format ###############################

# create column structure from example in WQX template. Use existing input from SGS results if applicable, specify value from "Permitted Values" tab if SGS input not applicable or not yet specified

# 6/21/23:: check to see if format will still work for EPA WQX template

# 10/12/2023 - there might not be a point to doing 'all' of this formatting at this stage, if we don;t have it flagged and ready for export yet

# 12/4/2023 - confirmed that it was unnecessary to have the column names formatted with capital letters & spaces at this stage.

# Current task - take the dataframe at the end of this long script, make so column names/structure conforms to the ADEC example. In the QA/QC process, it went through the janitor::clean_names process, thus names need to be fixed again at the end.

# Save raw-column-name dat before WQX renaming, so downstream QA/QC chunks
# (which expect original column names like analyte, collect_date, etc.) continue to work.
dat_raw <- dat
...
# next, for our WQX export, we want to retain just those columns listed in the WQX template
# found solution to this problem here: https://gist.github.com/djhocking/62c76e63543ba9e94ebe

# get all column names from WQX template
wqx_colnames <- read_excel("other/input/wqx_templates/AWQMS_KWF_Baseline_2021.xlsx", sheet = "KWF Baseline AWQMS Template") %>%
  colnames()

# select from subset of columns in WQX template
dat %<>%
  select(one_of(wqx_colnames))
 
# export final WQX-formatted results to external csv
write.csv(dat,"other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv",row.names = F)


z <- dat %>%
  filter(`Characteristic Name` %in% c("Magnesium","Calcium","Calcium, Total","Magnesium, Total"),
         `Activity Start Date` == "2021-5-11")
...
```{r, echo = F}
embed_if_html('other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv', text = "Download All Provisional 2021 Kenai River Baseline Water Quality Monitoring Results, Formatted for EPA WQX import")


# check unique activity types
#z <- read.csv("other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv")
...
total_samples_collected_2021_summary_param <- read.csv('other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv') %>%
  clean_names() %>%
...
planned_samples_2021_summary_by_site <- planned_samples_2021 %>%
...
total_samples_collected_2021_summary <- read.csv('other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv') %>%
  clean_names() %>%
...
export_dat <- read.csv('other/output/wqx_formatted/intermediate/2021_kwf_baseline_results_wqx.csv') %>%
  clean_names()
...
write.csv(matrix_spike_recovery_fails_tbl,"other/output/lab_qaqc_data/matrix_spike_recovery_fails_2021.csv")
...
# embed for download
embed_if_html("other/output/lab_qaqc_data/matrix_spike_recovery_fails_2021.csv", text = "Download Matrix Spike Recovery Values Exceeding Threshold for 2021 Sample Analyses")
...
# this step is already accomplished in a previous code block, which imports and joins "other/input/wqx_templates/2021_data_flag_decisions.csv" to the parameters chosen in this discussion.
...
# Apply data flags and write the flagged export CSV before completeness measures are calculated.
# This chunk must run before Q25 and Q26, which both read from this file.
...
# NOTE: this csv also has a column for summary rationale for each flagging decision
flags_2021 <- read.csv('other/input/wqx_templates/2021_data_flag_decisions.csv') %>%
  select(-notes) %>%
  transform(activity_start_date = mdy(activity_start_date))

export_dat <- full_join(export_dat, flags_2021) %>%
...
write.csv(export_dat, "other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv", row.names = F)

```

```{r echo = F, message=FALSE}
# read in list of analytes specified in QAPP
analytes_2021 <- read.csv("other/input/wqx_templates/analytes_list_manual_edit.csv")
colnames(analytes_2021) <- c("characteristic_name","a","param_in_qapp") 
analytes_2021 %<>% select(-a)

# BY PARAMETER
# summarise flags by parameter
completeness_summary_param <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
...
completeness_summary_site <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
completeness_summary_project <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
cma_dat_inline <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') |>
  clean_names() |>
  left_join(analytes_2021) |>
  filter(param_in_qapp == "Y")
...
cmb_by_method <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
  ...
  rename(flag_Y = Y,
         flag_N = N) %>%
  transform(activity_start_date = ymd(activity_start_date)) %>%
  left_join(planned_samples_2021_summary) %>%
...
planned_by_site_totals <- planned_samples_2021_summary_by_site %>%
  group_by(monitoring_location_id) %>%
  summarise(expected_results_n = sum(expected_results_n), .groups = "drop")

cmb_by_site <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  # reduce the list of analytes to only those specified in QAPP
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
...
total_planned_2021 <- sum(planned_samples_2021_summary$expected_results_n)

cmb_project <- read.csv('other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv') %>%
  clean_names() %>%
  left_join(analytes_2021) %>%
  filter(param_in_qapp == "Y") %>%
  group_by(flag) %>%
...
export_dat <- read.csv("other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv")
...
# re-write export_dat
write.csv(export_dat,"other/output/wqx_formatted/intermediate/2021_export_data_flagged.csv")
...
# save
dir <- "other/output/misc/characteristic_count_2021.csv"
...
# NOTE: The flagging chunk that generates 2021_export_data_flagged.csv has been moved
# to just before Q25 (Completeness Measure A) so that both CMA and CMB calculations
# read from a freshly-written version of the file within the same render pass.
# export_dat (with flag column) remains in memory from that earlier chunk.
...
# note all flagged observations in the 2021 dataset

# label all flagged observations as "Rejected" and non-flagged as "Final."
...
# was planned and approved. The 200.8 phosphorus results are retained in the
# flagged intermediate file but are not submitted to CDX.
export_dat %<>%
...
# here, we will re-develop the "export_dat" dataframe to match desired import configuration for EPA WQP; modify import config as needed. 

# before we prepare the dataframe, we need to re-prepare some columns in order to correctly create the "Activity ID". In the future we can avoid this step by using desired site names in earlier step

# --> site names
# rejoin WQP site location info (site names) to export_dat
## use matched data at "other/input/wqx_templates/wqx_template_matching_table.xlsx
site_name_match <- read_excel("other/input/wqx_templates/wqx_template_matching_table.xlsx", sheet = "adec_site_names") %>%
  remove_empty()
export_dat <- left_join(export_dat, site_name_match)

# --> characteristic names
# rejoin edited list of analyte abbreviation names, then append to dataframe. Use in creating Activity IDs.
analyte_abbrev <- read.csv("other/input/wqx_templates/analytes_list_manual_edit.csv") 
...
# --> sample_condition names (dup or field blank)
# this column is not present, so we will extract the info from the existing activity_id column
export_dat %<>%
  mutate(sample_condition_abbrv = case_when(
...
# --> finalize dataframe
# Proceeding left to right across columns of AWQMS template
# Mutated new column or rename existing column as needed
export_dat %<>%  
   mutate(
    `Project ID` = 10000007,
    ...
    `Result Detection Condition` = result_detection_condition,
    # Detection limit columns are numbered "1" and "2" to avoid duplicate column names in R;
    # the CDX import configuration maps these to the two allowed detection limit slots.
...
# export final WQX-formatted results to external csv
write.csv(export_dat,"other/output/wqx_formatted/results_activities.csv",row.names = F)
...
#   - Keep only the Agency Baseline project row (this submission does not include Hydrocarbons)
#   - Update QAPP approval status and approving agency (QAPP v3 approved by EPA Region 10, May 2023)
#   - Set ProjectDescriptionText to the project home page URL
#   - Attach the QAPP PDF filename and type (uploaded alongside this CSV in CDX)
#   - Drop columns not used in the CDX import configuration
...
# Well/aquifer columns and provider metadata are excluded - not applicable to this project.
...
station_upload <- read.csv("other/input/WQX_downloads/station/station.csv") |>
  ...
  # "Interpolation-Satellite" is not a valid WQX domain value for
  # HorizontalCollectionMethodName. WQX substitutes "Unknown" and drops the
  # HUCEightDigitCode, which prevents stations from appearing in How's My Waterway.
  # "GPS-Unspecified" is the correct WQX value for satellite-based coordinates
  # and is what ADEC uses for nearby stations that do appear in HMW.
  # Both "Interpolation-Satellite" and "Unknown" are corrected here as a safety net
  # in case either value appears in a future WQP re-download.
