...
-   Dissolved metals results are ***not*** consistently less than total metals results, as would be anticipated (see [Flowlink](https://www.flowlink.ca/post/2018/12/15/dissolved-total-metals-whats-the-difference-and-why-it-matters) website for description and rationale.)
...
A csv file of RPD values organized by site, date, and parameter may be downloaded below:
...
In the 2020 QAPP, RPD values for most parameters are set at 20%, with the exception of "Phosphorus, total" and "Nitrates (NO2+NO3)" at 25%, and "Suspended solids, total" at 5%.
...
Laboratory discrepancies from the SGS lab are described on page 2 of the spring and summer reports, which are available for download below.
...
Based on the data flagging decisions discussed in the previous questions, the 2021 dataset did not achieve all Completeness Measure A goals. By parameter, all dissolved metals had a value of 0% for Completeness Measure A since the decision was made to flag these as a result of an inability to calculate Relative Percent Difference values. By Site, Completeness Measure A values ranged from 44.4% - 71.4%, depending on the site's sampling plan. For the overall 2021 dataset, Completeness Measure A is calculated at 50.1%; indicating that 50.1% of the overall result values were flagged.
...
cmb_by_site <- read.csv('other/output/aqwms_formatted_results/2021_export_data_flagged.csv') %>%
  ...
  # WORKING HERE: 8/24/2023:::format for planned sample summary df does not include site names yet. create df that includes site names
