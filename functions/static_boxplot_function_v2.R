...
create_facet_plots <- function(data_path, reg_vals_path, characteristic, sample_fraction = character(0)) {
  ...
  create_plot <- function(subset_data, x_var) {
    ...
    has_exceedance <- any(!is.na(subset_data$fw_acute_exceed) | !is.na(subset_data$fw_chronic_exceed))
    
    # Check if there are any regulatory lines present
    has_hline <- nrow(hline_data) > 0
    
    ggplot(subset_data, aes(x = get(x_var), y = result_measure_value)) +
      geom_boxplot(aes(group = get(x_var)), outlier.shape = NA) +
      
      # --- MODIFIED CODE ---
      # Conditionally show geom_jitter legend only if there are valid exceedance values
      geom_jitter(aes(
      ...
      ), width = 0.2, size = 3, show.legend = has_exceedance) +
      
      # --- MODIFIED CODE ---
      # Conditionally show geom_hline legend only if there are valid regulatory lines
      geom_hline(data = hline_data, aes(yintercept = yintercept, linetype = linetype),
                 color = "#D55E00", size = 1.2, show.legend = has_hline) +
      ...
      scale_color_manual(
        ...
        values = c(
          "gray" = "#7F7F7F",    # Gray for None
          "orange" = "#E69F00",  # Orange for Chronic
          "blue" = "#0072B2"      # Blue for Acute
        )
      ) +
      scale_shape_manual(
        name = "Hardness Dependent\nExceedance Type",
        ...
        labels = c("gray" = "None", "orange" = "Chronic", "blue" = "Acute"),
        values = c(
          "gray" = 16,    # No exceedance
          "orange" = 16,  # Chronic exceedance
          "blue" = 8      # Acute exceedance
        )
      ) +
