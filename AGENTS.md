...
2.  **\[HIGH PRIORITY\] Fix HMW visibility for 15 legacy numeric-ID stations - move process to `kenai-river-wqx-qaqc` repo and carry out the upload.** During session 3/31/2026, a corrective station upload file was generated at `other/output/epa_wqp_uploads/corrected_epa_wqp_uploads/station_historical_correction.csv` for 15 historical stations (IDs `KENAI_WQX-10000063` through `KENAI_WQX-10000131`, named `KBX_m_*`, covering 2002–2009, 3,884 results). These stations have `HorizontalCollectionMethodName = "Unknown"` and blank `HUCEightDigitCode` in WQP - the same root cause as the 22 current stations, but they are not covered by the `station.csv` re-upload in Task 1. **This process and its documentation belong in `kenai-river-wqx-qaqc`, not here.** Action items: (a) Move or recreate the corrective upload script in `kenai-river-wqx-qaqc`. The logic is simple: query `whatWQPsites(organization = "KENAI_WQX")`, filter to the 15 numeric IDs, set `HorizontalCollectionMethodName = "GPS-Unspecified"` and `HUCEightDigitCode = "19020302"`, write to CSV matching the CDX station upload format. (b) Upload the corrective file to CDX (same import path as a regular station file - no deletion needed, WQX updates in place). (c) Verify in WQP that the 15 stations now have populated HUC codes. (d) Delete `station_historical_correction.csv` from this repo once the process is documented and executed in `kenai-river-wqx-qaqc`. **Note:** At the end of the 3/31/2026 session, it was observed that some KWF sites with historical data are already appearing in HMW *before* any corrective steps were taken. The full explanation for this is not yet clear - it may be that WQX partially populates HUC codes for some stations, or that HMW has other lookup paths. Investigate when picking up this task.
