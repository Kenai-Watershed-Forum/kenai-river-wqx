...
# Retain only total phosphorus fractions. "Filtered, field" rows are dissolved P
# from the SGS 200.8 dissolved metals run and are excluded (that method does not
# specify phosphorus speciation; those results are Rejected in the CDX export).
# "Total Residual" is a single-row anomaly also excluded.
# Historical records with NA fraction are retained — same method used throughout.
sample_fraction <- c("Total", "Total Recoverable")
