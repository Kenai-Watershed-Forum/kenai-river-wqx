...
For total aromatic hydrocarbons, the Alaska Dept of Environmental Conversation describes a maximum static regulatory value of 10 ug/l.

To prepare our existing data to be assessed against this standard, we addressed the following issues:

-   In some cases, BTEX is quantified as a total mixture. In other cases, the individual substances are quantified separately. In the latter case, we summed the total quantities of benzene, toluene, ethylbenzene, and xylene by unique sampling event (unique location/time).
...
# However, today it is unclear which specific parameters, or grouped sums of parameters, have equivalent names in our data set downloaded from the EPA WQX.

# Here, we will structure data to simply assess BTEX values against the ADEC regulatory standard. There may be federal standards that also appy, to be addressed later.
...
# to read this data source in separately, try using case_when within read.csv to choose oneof two data sources
