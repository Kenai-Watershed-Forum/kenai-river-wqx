...
hard <- dat %>%
  ...
  # retain onl;y total Ca and Mg, not dissolved values
...
# working here 2/13/2025 :: makle sure that 2021 hydrocarbon data aligns with 2000 - 2013
...
# %>%
  ...
  # this filter is not including 2021 data fior unknown reason

z <- dat %>%
  transform(activity_start_date <- ymd(activity_start_date)) %>%
  filter(activity_start_date > ymd("2021-01-01")) %>%
  distinct(characteristic_name)
  

# 2021 hydrocarbon data is missing from here. see appendix a, investigate why
# does the hydrocarbon data  accidentally get eliminated in this reg_limits script, or is it not exported in appendix a???
# in appendix a, 2021 hydrocarbon data contains: ethylbenzene, P & M -Xylene, o-Xylene, Benzene, Toluene
