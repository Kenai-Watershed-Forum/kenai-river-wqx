...
btex_dat <- dat %>%
  ...
  mutate(match = case_when(
    ...
    TRUE ~ "N")) %>%
  # for instances where no provided value exists, use calculated value
  ...
  # types of issues addressed in preparing hydrocarbon data

# - value exists only for provided sum column (we left these as is)
# - no provided value exists, only calculated (we used the calculated value)


# next, replace old volatiles data with the dataframe above. rejoin to site info. task for 1/9/2025
