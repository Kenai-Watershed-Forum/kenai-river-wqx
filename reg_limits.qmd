...
# WORKING HERE!!!!
# 9/30/24 the join below causes hardness values to convert to NA
# spent an hour with chatcpt trying to diagnose why hardness values are converted to NA in the join. this step is essential to we can calculate hardness-dependent values. next: try performing join(s) one parameter at a time to see if the issue is parameter specific?!?!?!?!?!??
...
hardness_dependent_metals_thresholds <- inner_join(dat, diss_metals_hard_param, by = "characteristic_name") %>%
