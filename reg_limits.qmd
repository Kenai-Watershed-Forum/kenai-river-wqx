...
Some regulatory values for metals are not simple static values, and are instead hardness-dependent (i.e. calcium and magnesium concentrations). Hardness varies by site, by season, and with hydrologic conditions. We calculated hardness according to the following formula:
...
hard <- dat %>%
  select(activity_identifier, activity_start_date, characteristic_name, result_measure_value, result_measure_measure_unit_code) %>%
  filter(characteristic_name %in% (c("Magnesium","Calcium"))) %>%
  group_by(activity_identifier, activity_start_date, characteristic_name, result_measure_measure_unit_code) %>%
...
dat <- full_join(dat,hard,by = "activity_identifier") 
...
These values are graphically displayed in the following chapters not juxtaposed against static horizontal lines, but instead they are highlighted by shape or color when the calculated threshold is exceeded; specific that sampling event's hardness conditions.
...
dat_metals_hard_dept <- dat %>%
  filter(characteristic_name %in% c("Arsenic","Cadmium","Chromium","Copper","Lead","Nickel","Silver","Zinc"))

# join calculation parameters to overall dataset
hardness_dependent_metals_thresholds <- full_join(dat_metals_hard_dept, diss_metals_hard_param, by = "characteristic_name") 

# join eliminates hardness values
# troubleshooting here
write.csv(dat_metals_hard_dept,"other/temp_dat.csv", row.names = F)
write.csv(diss_metals_hard_param,"other/temp_diss_hard.csv", row.names = F)
...
# no threshold values calculated
