...
# WORKING HERE 1/7/2025
# 1) summarise hydrocarbons
# 2) modify boxplot to read in seperate data source for btex
...
btex_dat <- dat %>%
  ...
  summarise(result_measure_value = mean(result_measure_value)) %>%
    # simplify multiple xylene isomers to just one general xylene
  #mutate(characteristic_name = str_replace_all(characteristic_name,xylene_isomers)) %>%
  #group_by(activity_identifier,characteristic_name) %>%
#  summarise(result_measure_value = mean(result_measure_value)) %>%
  
  # sum quantities of BTEX molecules for sampling instances where they are quantified separately
 # group_by(activity_identifier,characteristic_name, .drop = F) %>%
  #summarise(result_measure_value = sum(result_measure_value)) %>%
  # # reviewing one of the above bullets: do all instances of the full BTEX value reflect the sum of individually reported parametres wihin??
  ...
  mutate(match = case_when(
    `Benzene, toluene, ethyl benzene, xylenes mix` == btex_sum ~ "Y",
    TRUE ~ "N"
    
  ))

  # types of issues to address
# - btex_sums match, but Y/N column gives erroneous answer ("easy" fix)
# - value exists only for provided sum column
# - no provided value exists, only calculated


# to read this data source in separately for boxplots, try using case_when within read.csv to choose one of two data sources
